...
from models.Layers import Graph_GRUModel
...
class CategoricalGraphAtt(nn.Module):
    def __init__(self, input_dim, window_size, output_dim, lstm_hidn_dim, intra_gat_hidn_dim, inter_gat_hidn_dim, inner_edge, outer_edge, company_to_sector, device):
        ...
        self.fusion = nn.Linear(lstm_hidn_dim+intra_gat_hidn_dim+inter_gat_hidn_dim, lstm_hidn_dim) # TAGAT_full
    ...
    def forward(self, daily_data_batch):
        # daily_data_batch should be renamed to time_stamp_data_batch
        # batch norm
        # print("Shape of daily_data_batch", daily_data_batch.shape)
        daily_data_batch = torch.permute(daily_data_batch, dims=(1, 0, 2))
        daily_data_batch = torch.reshape(daily_data_batch, (-1, self.window_size * self.input_dim))
        ...
        fusion_vec = torch.cat((sequence_embeddings, sectors_embeddings, intra_sector_embeddings), dim=-1) # TAGAT_full
...
class CategoricalGraphAtt_wo_intra(nn.Module):
    def __init__(self, input_dim, window_size, output_dim, lstm_hidn_dim, intra_gat_hidn_dim, inter_gat_hidn_dim, inner_edge, outer_edge, company_to_sector, device):
        super(CategoricalGraphAtt_wo_intra, self).__init__()
        ...
        self.fusion = nn.Linear(lstm_hidn_dim+inter_gat_hidn_dim, lstm_hidn_dim) # TAGAT_wo_intra
    ...
    def forward(self, daily_data_batch):
        ...
        sequence_embeddings = self.sequence_encoder(daily_data_batch_norm)
        print("Shape of sequence_embeddings:", sequence_embeddings.shape)
        # print("Shape of sequence_embedding:", sequence_embeddings[0].shape)

        ## Conduct intra-sector embedding
        intra_sector_embeddings = self.intra_gat(sequence_embeddings, self.inner_edge)
        print("Shape of intra_sector_embeddings:", intra_sector_embeddings.shape)
        # print("Shape of intra_sector_embedding:", intra_sector_embeddings[0].shape)
        ...
        sectors_embeddings = torch.stack(sectors_embeddings, dim=0) # (num_sectors, hidden_dim)
        print("Shape of sectors_embeddings:", sectors_embeddings.shape)
        # print("Shape of sectors_embedding:", sectors_embeddings[0].shape)
        ...
        fusion_vec = torch.cat((sequence_embeddings, sectors_embeddings), dim=-1) # TAGAT_wo_intra
        print("Shape of fusion_vec", fusion_vec.shape)
        # print("Shape of fusion_vec", fusion_vec[0].shape)
...
class CategoricalGraphAtt_wo_inter(nn.Module):
    def __init__(self, input_dim, window_size, output_dim, lstm_hidn_dim, intra_gat_hidn_dim, inter_gat_hidn_dim, inner_edge, outer_edge, company_to_sector, device):
        super(CategoricalGraphAtt_wo_inter, self).__init__()
        ...
        self.fusion = nn.Linear(lstm_hidn_dim+intra_gat_hidn_dim, lstm_hidn_dim) # TAGAT_wo_inter
    ...
    def forward(self, daily_data_batch):
        ...
        fusion_vec = torch.cat((sequence_embeddings, intra_sector_embeddings), dim=-1) # TAGAT_wo_inter
