...
type='index'
exp_dir=$ROOT/experiments/gru12_index_wd$3
...
if $init_experiment; then
    ...
    do
        ...
        cp $ROOT/experiments/gru_index/params.json $exp_dir/${type}_fold_${i}
...
python ./PyTorch_Ver_dev/main.py --model_dir=$dir --data_dir=$data_dir/${type}/${type}_fold_${i} --label_type cum --device=$device --num_epochs=$num_epochs --learning_rate=$learning_rate --weight_decay=$weight_decay --batch_size=$batch_size --gamma=$gamma --lstm_num_units=$lstm_num_units &&
# make sure data_file is valid_subset_cum.gz
python ./PyTorch_Ver_dev/predict.py --model_dir=$dir --data_dir=$data_dir/${type}/${type}_fold_${i} --data_file test_cum.gz --restore_dir last_weights --device cpu &&
# make sure h_params are given, mode as a for append
python ./PyTorch_Ver_dev/report.py --pred_file $dir/pred_test.csv --label_file $data_dir/${type}/${type}_fold_${i}/test_cum.gz --max_epoch=$max_epoch --patience=$patience --learning_rate=$learning_rate --weight_decay=$weight_decay --batch_size=$batch_size --gamma=$gamma --lstm_num_units=$lstm_num_units