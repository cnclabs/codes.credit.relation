...
@click.option('--max_epoch', default=None, type=int)
@click.option('--patience', default=None, type=int)
@click.option('--batch_size', default=None, type=int)
@click.option('--learning_rate', default=None, type=float, help='1e-3, 1e-4, 1e-5')
@click.option('--weight_decay', default=None, type=float, help='1e-4, 1e-5, 1e-6, l2 regularization')
@click.option('--gamma', default=None, type=float, help='exponential learning rate scheduler, lr = lr_0 * gamma ^ epoch')
@click.option('--lstm_num_units', default=None, type=int, help='lstm_num_units')

def main(pred_file, label_file, max_epoch, patience, batch_size, learning_rate, weight_decay, gamma, lstm_num_units):
    ...
    h_params = {'max_epoch': max_epoch, 'patience': patience, 'batch_size': batch_size, 'learning_rate': learning_rate, 'weight_decay': weight_decay, 'gamma': gamma, 'lstm_num_units': lstm_num_units}
    save_dict_to_json(metrics, json_file, h_params=h_params)
