...
def model_fn(is_training, inputs, params, reuse=False):
    ...
    if is_training:
        ...
        ## cwlin: Wrap optimizer with learning rate decay mechanism
        global_step = tf.train.get_or_create_global_step()
        lr_schedule = tf.keras.optimizers.schedules.ExponentialDecay(
                    initial_learning_rate=params.learning_rate,
                    decay_steps=params.decay_steps,
                    decay_rate=params.decay_rate,
                    staircase=True)(global_step) # global_step is passed to the ExponentialDecay function, which returns a callable that will compute the decayed learning rate.
