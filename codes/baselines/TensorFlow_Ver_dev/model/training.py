...
def train_and_evaluate(train_model_spec, eval_model_spec, model_dir, params, restore_from=None):
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(begin_at_epoch, begin_at_epoch + params.num_epochs):
            ...
            ## cwlin
            # Patience: stop training if no improvement is seen in last patience epochs.
            if params.validation:
                if metrics['loss'] < min_valid_loss:
                    min_valid_loss = metrics['loss']
                    patience = 0
                else:
                    patience += 1
        ...
        if params.validation:
            if final_num_epoch == 0:
                # meaning that the training is not stopped by early stopping
                final_num_epoch = params.num_epochs
            else:
                # meaning that the training is stopped by early stopping
