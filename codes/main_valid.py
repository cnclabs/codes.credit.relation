...
if __name__ == '__main__':
    ...
    start_t = time.time()
    train_inputs, _ = load_dataset(True, train_data, args.batch_size, FEATURE_SIZE, WINDOW_SIZE, COMPANY_IDS)
    valid_inputs, _ = load_dataset(True, valid_data, args.batch_size, FEATURE_SIZE, WINDOW_SIZE, COMPANY_IDS)
    end_t = time.time()
    ...
    final_save_path = os.path.join(model_dir, 'last_weights', f'train_valid_{args.model_name}_checkpoint.pt')
    ...
    for epoch in range(args.max_epoch):
        ...
        train_ret_metric = train(model, CUM_LABELS, train_inputs, optimizer, criterion, device)
        valid_ret_metric = evaluate(model, CUM_LABELS, valid_inputs, criterion, device)
    ...
    # Save stop at epcoh
    with open(f'{model_dir}/num_epochs', 'w') as f:
